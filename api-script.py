...
api = api.strip() # stripping any accidental whitespace from the API key
...
if response.status_code == 200: # only proceed if the HTTP request succeeds
    ...
    for pages in range(starting_page, allPages + 1):
        step_response = sesh.get(f"https://api.pokemontcg.io/v2/cards?page={pages}&pageSize=250", headers = headers, timeout = (10.0, 60.0)) # a new response URL to include both page number and size
        try:
